...
export async function sendMatchApplicationEmails(
  ...
  const targets = [
    ...recipientsOf(hostSchool).map((r) => ({ ...r, subject: hostSubject, html: hostHtml })),
    ...recipientsOf(applicantSchool).map((r) => ({ ...r, subject: applicantSubject, html: applicantHtml })),
  ];
